...
MY ANSWER: I think this algorithm is greedy because it makes the locally optimal choice for each piece of the input text. I don't think this algorithm is recursive, and as a result, it doesn't count as dynamic programming (this algorithm doesn't use its solution to any sub-problems to formulate a solution ot another problem).

b. If the text contains a word that is longer than `max_width`, what does this algorithm do?

MY ANSWER: The algorithm simply prints the entire word on a new line.

c. Does this algorithm produce an optimal result? (Do you have enough information to answer this question?)

MY ANSWER: What is an optimal result...? According to Wikipedia, greedy algorithms usually (but not always) do not return the globally optimal result. In this case, though, what is the optimal result? 

Oh, this is what the greedy algorithm does:
...
And this is what the optimal solution would be:
...
I sort of understand this. So, no, this algorithm doesn't produce the optimal result.
...
MY ANSWER: This algorithm is dynamic programming (top down) because it uses its solution to some sub-problems to calculate a solution to other problems (as exemplified by the candidates array).
...
MY ANSWER: wow he seems like a k00l d00d
